...
// Filter Mode
typedef NS_ENUM(NSUInteger, SPFragmentFilterMode)
{
...
/** ------------------------------------------------------------------------------------------------
 ...
 A fragment filter works in the following way:
    ...
    Each pass processes the texture using a fragment shader (and optionally a vertex
        shader) to achieve a certain effect.
    The output of each pass is used as the input for the next pass; if it's the
        final pass, it will be rendered directly to the back buffer.
 
 All of this is set up by the abstract FragmentFilter class. Concrete subclasses
 ...
 This can be done through the 'mode' property, which accepts one of the enums defined
 in the 'SPFragmentFilterMode' enum.
 
 Beware that each filter should be used only on one object at a time. Otherwise, it
...
@property (nonatomic) SPFragmentFilterMode mode;
