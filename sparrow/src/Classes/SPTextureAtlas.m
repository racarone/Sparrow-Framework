...
- (void)dealloc
{
    SP_RELEASE_AND_NIL(_atlasTexture);
    SP_RELEASE_AND_NIL(_path);
    SP_RELEASE_AND_NIL(_textureRegions);
    SP_RELEASE_AND_NIL(_textureFrames);
...
- (void)parseAtlasXml:(NSString*)path
{
    ...
    BOOL success = [parser parseElementsWithBlock:^(NSString* elementName, NSDictionary* attributes)
    {
        ...
        else if (!_atlasTexture && [elementName isEqualToString:@"TextureAtlas"])
        {
            // load atlas texture
