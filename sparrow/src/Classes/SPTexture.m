                    ...
                    premultipliedAlpha:(BOOL)pma
{
    ...
    SP_RELEASE_AND_NIL(self);
    self = [[SPGLTexture alloc] initWithTextureInfo:info
                                              scale:[fullPath contentScaleFactor]
...
{
    // only textures with side lengths that are powers of 2 support all OpenGL ES features.
    ...
    SP_RELEASE_AND_NIL(self);
    self = [[SPTexture alloc] initWithRegion:region ofTexture:glTexture];
    return self;
}
...
- (instancetype)initWithRegion:(SPRectangle*)region frame:(SPRectangle*)frame ofTexture:(SPTexture*)texture
{
    ...
    {
        SP_RELEASE_AND_NIL(self);
        self = [[SPSubTexture alloc] initWithRegion:region frame:frame ofTexture:texture];
    }
    else
    {
        SP_RELEASE_AND_NIL(self);
...
{
    return [[(SPTexture*)[self alloc] initWithContentsOfFile:path] autorelease];
}

+ (instancetype)textureWithContentsOfFile:(NSString*)path generateMipmaps:(BOOL)mipmaps
{
    return [[(SPTexture*)[self alloc] initWithContentsOfFile:path generateMipmaps:mipmaps] autorelease];
}

+ (instancetype)textureWithRegion:(SPRectangle*)region ofTexture:(SPTexture*)texture
{
    return [[(SPTexture*)[self alloc] initWithRegion:region ofTexture:texture] autorelease];
}

+ (instancetype)textureWithWidth:(float)width height:(float)height draw:(SPTextureDrawingBlock)drawingBlock
{
    return [[(SPTexture*)[self alloc] initWithWidth:width height:height draw:drawingBlock] autorelease];
}
...
{
    return [self emptyTextureWithWidth:width height:height generateMipmaps:false];
}

+ (instancetype)emptyTextureWithWidth:(float)width height:(float)height generateMipmaps:(BOOL)mipmaps
{
    return [self emptyTextureWithWidth:width height:height generateMipmaps:false premultipliedAlpha:true];
}
...
+ (instancetype)emptyTextureWithWidth:(float)width height:(float)height generateMipmaps:(BOOL)mipmaps premultipliedAlpha:(BOOL)pma scale:(float)scale
{
    ...
    NSInteger origWidth = width * scale;
    NSInteger origHeight = height * scale;
    NSInteger potWidth = [SPUtils nextPowerOfTwo:origWidth];
    NSInteger potHeight = [SPUtils nextPowerOfTwo:origHeight];
    BOOL isPot = (origWidth == potWidth && origHeight == potHeight);
    BOOL useRectTexture = !isPot && !mipmaps;
    ...
    glTexture = [[[SPGLTexture alloc] initWithData:NULL width:actualWidth height:actualHeight
                                   generateMipmaps:mipmaps scale:scale premultipliedAlpha:pma] autorelease];

    if (isPot || useRectTexture)
        return glTexture;
    else
        return [self textureWithRegion:[SPRectangle rectangleWithX:0 y:0 width:width height:height] ofTexture:glTexture];
}
